	...
	//	$json .= ',';
	 //   }
		...
		$prec[] = array(
				...
				'<a href="javascript:;" class="lineitems_link" id="'.$v->id . '" rel="' . $v->invoice_number . '">'.$v->invoice_number.'</a>',
				$v->invoice_date->format('M d, Y'),
				$this->ct->view->BUtils()->getCurrency($v->amount_due),
				$this->ct->view->BUtils()->getCurrency($v->amount_paid),
				$this->ct->view->BUtils()->getCurrency($v->amount_due - $v->amount_paid),
				'&nbsp;&nbsp;<a href="javascript:;" class="payment_link" id="' . $v->id . '" rel="' . $v->invoice_number . '">Payment</a>'
