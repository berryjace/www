...
class Client_Form_Signature extends Zend_Form {

    public function init() {
        $this->setName('sign_form');
        $this->setAttrib('enctype', 'multipart/form-data');

        $client_signature = new Zend_Form_Element_Text('client_signature');
        $client_signature->setDisableLoadDefaultDecorators(true)
                ->setDecorators(array('ViewHelper', 'Errors'))
                ->setValidators(array(array('NotEmpty', true, array('messages' => 'Please enter signature'))))
                ->setAttrib('class', 'text')
                ->setAttrib('id', 'client_signature')
                ->setAttrib('name', 'client_signature')
                ->setAttrib('size', '20')
                ->setRequired(true)
                ->addFilter('StripTags')
                ->addFilter('StringTrim');

        $client_title = new Zend_Form_Element_Text('client_title');
        $client_title->setDisableLoadDefaultDecorators(true)
                ->setDecorators(array('ViewHelper', 'Errors'))
                ->setValidators(array(array('NotEmpty', true, array('messages' => 'Please enter title'))))
                ->setAttrib('class', 'text')
                ->setAttrib('id', 'client_title')
                ->setAttrib('name', 'client_title')
                ->setAttrib('size', '20')
                ->setRequired(true)
                ->addFilter('StripTags')
                ->addFilter('StringTrim');
        ...
        $recom_for_client->setLabel('Recommendation from Affinity')
                ->setDisableLoadDefaultDecorators(true)
                ->setAttribs(array('cols' => '135', 'rows' => '4', 'class' => 'textarea'))->setRequired(true)
