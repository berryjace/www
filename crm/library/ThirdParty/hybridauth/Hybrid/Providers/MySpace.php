...
class Hybrid_Providers_MySpace extends Hybrid_Provider_Model
{
	...
	*/
	function initialize() 
	{
		...
		if ( $this->token( "access_token" ) && $this->token( "access_secret" ) )
		{
			$this->api = new MySpace
						(
							$this->config["keys"]["key"], $this->config["keys"]["secret"],
							$this->token( "access_token" ), $this->token( "access_secret" )
						); 
		}
	}
	...
	*/
	function loginBegin()
	{
		...
		if ( ! isset( $tokz['oauth_token'] ) || ! is_string( $tokz['oauth_token'] )
	...
	*/
	...
	{ 
		...
		$this->token( "access_secret" , $tokz->secret ); 
