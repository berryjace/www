...
class Hybrid_Provider_Adapter
{
	...
	*/
	...
	function login()
	{
		...
		$this->params["login_start"] = $HYBRID_AUTH_URL_BASE . ( strpos( $HYBRID_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.start={$this->id}&hauth.token={$this->params["hauth_token"]}&hauth.time={$this->params["hauth_time"]}";
	...
	*/
	function user()
	{
		if( ! $this->adapter->isUserConnected() ){
			Hybrid_Logger::error( "User not connect while trying to access his data on {$this->id}." );

			throw new Exception( "User not connect while trying to access his data {$this->id}." );
		}
	...
	*/
	...
	{
		if( ! $this->adapter->isUserConnected() ){
			Hybrid_Logger::error( "User not connect while trying to access {$this->id} API." );

			throw new Exception( "User not connect while trying to access {$this->id} API." );
		}
