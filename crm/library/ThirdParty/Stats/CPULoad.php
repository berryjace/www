...
//					$this->print_current();
    ...
    function print_current() {
        printf("Current load tickers - User: %f  Nice: %f  System: %f  Idle: %f<br>", $this->user, $this->nice, $this->system, $this->idle
        );
    }

    function print_load() {
        printf("User: %.1f%%  Nice: %.1f%%  System: %.1f%%  Idle: %.1f%%  Load: %.1f%%<br>", $this->load["user"], $this->load["nice"], $this->load["system"], $this->load["idle"], $this->load["cpu"]
        );
    }

    function print_load_json() {
        return "
            ['User',    ".number_format($this->load['user'],2)."],
            ['Nice',    ".number_format($this->load['nice'],2)."],
            ['System',  ".number_format($this->load['system'],2)."],
            ['Idle',    ".number_format($this->load['idle'],2)."],
            {   'name'  : 'CPU',
                'y'     : ".number_format($this->load['cpu'],2).",
                'sliced' : true,
                'selected' : true
            }
        ";
