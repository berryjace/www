...
class VendorCorrespondenceRepository extends EntityRepository {
     ...
     * @return String
     */
    public function getCorrespondences($params) {
        ...
        $vendor_condition = (false !== $vendor_id) ? ' vc.id=?1' : '';

        $search_clause = !empty($searchStr) ? " WHERE vc.note like '%$searchStr%' AND vc.vendor_id={$vendor_id} " : "";
        $search_clause.= ($search_clause == "" AND false !== $vendor_id) ? 'WHERE vc.vendor_id=' . $vendor_id : '';
        ...
        $q = $this->_em->createQuery("
            SELECT vc
            FROM BL\Entity\VendorCorrespondence vc
            $search_clause
            $order_clause
       ");
...
//        $q->useResultCache(true);
    ...
    /**
     * Function to search vendors by correspondence information
     * @author Mahbub
     * @copyright Blueliner Marketing
     * @version 0.1
     * @access public
     * @return String
     */
    public function searchVendorByCorrespondence($search_params, $limit_fields=array()) {
